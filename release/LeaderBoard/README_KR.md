...
3. 만약 씬 내에서 제페토 캐릭터를 사용하지 않는다면 EventSystem을 추가해주세요.   
    > **주의** : ZepetoPlayers는 런타임에서 Event System을 생성하여 리더보드 버튼이 제대로 활성화 될수 있도록 합니다. Event System이 없다면 리더보드 버튼이 제대로 활성화 되지 않습니다.  
...
5. 씬안에 추가된 Prefab의 LeaderBoardPanel LeaderboardManager 스크립트의 LeaderboardId 입력칸에 복사한 리더보드 ID를 붙여넣습니다. 또한 Reset Rule에 해당 리더보드의 Reset Rule을 동일하게 선택합니다.
